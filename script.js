...
/* Hideable / Toggleable Unified Header & Toolbar */
function initUnifiedHeaderToggle() {
    const isHidden = localStorage.getItem("unified_header_hidden") === "true";
    setUnifiedHeaderVisibility(!isHidden);
}

function toggleUnifiedHeader() {
    const header = document.getElementById("unified-portal-header");
    if (!header) return;
    const isHidden = header.classList.contains("header-hidden");
    setUnifiedHeaderVisibility(isHidden); // If currently hidden, show it; else hide it
}

function setUnifiedHeaderVisibility(show) {
    const header = document.getElementById("unified-portal-header");
    const restoreBtn = document.getElementById("btn-show-unified-header");
    const hideBtn = document.getElementById("btn-hide-unified-header");
    ...
    if (show) {
        ...
        if (hideBtn) {
            hideBtn.innerHTML = '<i class="fa-solid fa-chevron-up"></i> <span>Hide Banner & Tools</span>';
            hideBtn.title = "Hide Banner and Toolbar (Shortcut: H)";
        }
        localStorage.setItem("unified_header_hidden", "false");
    } else {
        ...
        if (hideBtn) {
            hideBtn.innerHTML = '<i class="fa-solid fa-chevron-down"></i> <span>Show Banner & Tools</span>';
            hideBtn.title = "Show Banner and Toolbar (Shortcut: H)";
        }
        localStorage.setItem("unified_header_hidden", "true");
    }
}

// Backward-compatible wrappers
function toggleMainHeader() { toggleUnifiedHeader(); }
function toggleUpperBanner() { toggleUnifiedHeader(); }
...
function initCustomAudioPlayer() {
    ...
    audio.addEventListener("error", (e) => {
        console.warn("Audio media noticed an event:", e);
    });

    // Seeking on progress container click & drag
    if (progressContainer) {
        ...
        progressContainer.addEventListener("touchstart", (e) => {
            if (e.touches && e.touches[0]) {
                const rect = progressContainer.getBoundingClientRect();
                const clickPos = Math.max(0, Math.min(1, (e.touches[0].clientX - rect.left) / rect.width));
                if (audio.duration && !isNaN(audio.duration)) {
                    audio.currentTime = clickPos * audio.duration;
                }
            }
...
function playPauseAudio() {
    ...
    if (audio.paused) {
        audio.play().then(() => {
            ...
            }).catch(e => console.log("Audio load retry notice:", e));
        });
    } else {
        audio.pause();
        if (playBtn) {
...
function skipAudio(seconds) {
    const audio = document.getElementById("audio-media-element");
    if (audio) {
        const target = (audio.currentTime || 0) + seconds;
        audio.currentTime = Math.max(0, Math.min(target, audio.duration || 9999));
    }
}
...
/* ==========================================================
   GLOBAL TEXT-TO-SPEECH (TTS) ENGINE & VOICE SPEED CONTROL
   ========================================================== */
...
function getTTSVoiceSpeed() {
    const saved = localStorage.getItem("tts_voice_speed");
    return saved ? parseFloat(saved) : 1.0;
}

function initTTSVoiceSpeed() {
    const speedSelect = document.getElementById("tts-voice-speed");
    if (speedSelect) {
        const saved = localStorage.getItem("tts_voice_speed") || "1.0";
        speedSelect.value = saved;
    }
}

function changeTTSVoiceSpeed(val) {
    const rate = parseFloat(val) || 1.0;
    localStorage.setItem("tts_voice_speed", rate.toString());
}

/**
 * Universal robust speech synthesis function
 * @param {string} text - text to speak
 * @param {Function} [onEndCallback] - optional callback when speech finishes or errors
 * @param {number} [customRate] - optional override speech rate
 */
function speakText(text, onEndCallback, customRate) {
    if (!('speechSynthesis' in window)) {
    ...
    const speed = customRate || getTTSVoiceSpeed();
    ...
    if (hasArabic) {
        ...
        const arVoice = availableVoices.find(v => v.lang && (v.lang.startsWith("ar") || v.lang.includes("Arabic")));
        if (arVoice) {
            utterance.voice = arVoice;
        }
    } else {
        utterance.lang = "en-US";
        const enVoice = availableVoices.find(v => v.lang && (v.lang.startsWith("en-US") || v.lang.startsWith("en-GB") || v.lang.startsWith("en")));
        if (enVoice) {
...
function initTTSSelectionListener() {
    ...
    document.addEventListener("mouseup", (e) => {
        if (ttsBtn && ttsBtn.contains(e.target)) {
            return;
        }
        setTimeout(() => {
            handleTextSelection(ttsBtn, e);
        }, 40);
    });
...
function handleTextSelection(ttsBtn, mouseEvent) {
    const selection = window.getSelection();
    const text = selection ? selection.toString().trim() : "";

    if (text && text.length > 0 && text.length < 1200) {
...
function speakCurrentOrSelectedText(e) {
    ...
    if (!('speechSynthesis' in window)) {
        alert("Text-to-Speech is not supported in this browser. Please use Chrome, Microsoft Edge, Safari, or Firefox.");
        return;
    }

    // Toggle: If currently speaking, stop it cleanly
    if (window.speechSynthesis.speaking) {
        window.speechSynthesis.cancel();
        resetTTSButtons(ttsBtn, headerTtsBtn);
        return;
    }
    ...
    if (!textToSpeak) {
        textToSpeak = "Text-to-Speech active. Please highlight any word or sentence to hear its pronunciation, or click any TTS button on the page.";
    }
    ...
    if (headerTtsBtn) {
        headerTtsBtn.classList.add("speaking");
        headerTtsBtn.innerHTML = '<i class="fa-solid fa-volume-high fa-beat"></i> <span>Speaking...</span>';
    }
...
function resetTTSButtons(ttsBtn, headerTtsBtn) {
    if (ttsBtn) {
    ...
    if (headerTtsBtn) {
        headerTtsBtn.classList.remove("speaking");
        headerTtsBtn.innerHTML = '<i class="fa-solid fa-volume-high"></i> <span>TTS</span>';
    }
}
...
document.addEventListener("keydown", (e) => {
    ...
    } else if ((e.key === "h" || e.key === "H" || e.key === "t" || e.key === "T") && (e.ctrlKey === false && e.metaKey === false && e.altKey === false)) {
        if (document.activeElement.tagName !== "INPUT" && document.activeElement.tagName !== "TEXTAREA" && document.activeElement.tagName !== "SELECT") {
            toggleUnifiedHeader();
        }
...
/* ==========================================================
   ORIENTATION SLIDES SIDE-SCROLLER & CONTINUOUS LOOP TOOL
   ========================================================== */
let currentIntroSlideIndex = 0;
const totalIntroSlides = 6;
let isIntroSlideAutoplay = true;
let continuousScrollSpeed = 0.85; // Slow readable speed in pixels per frame
let continuousScrollAnimId = null;
let isSliderHoveredOrInteracting = false;

function slideIntroImages(direction) {
    currentIntroSlideIndex = (currentIntroSlideIndex + direction + totalIntroSlides) % totalIntroSlides;
    goToIntroSlide(currentIntroSlideIndex);
}

function goToIntroSlide(index) {
    currentIntroSlideIndex = index;
    const wrapper = document.getElementById("slides-scroll-wrapper");
    const cards = document.querySelectorAll(".slide-card");
    const dots = document.querySelectorAll(".slides-pagination .slide-dot");
    const counter = document.getElementById("intro-slide-counter");

    if (cards && cards[index] && wrapper) {
        const targetCard = cards[index];
        const scrollPos = targetCard.offsetLeft - (wrapper.clientWidth - targetCard.clientWidth) / 2;
        wrapper.scrollTo({
    ...
    if (counter) {
        counter.innerText = `Slide ${index + 1} / ${totalIntroSlides}`;
    }
}

function toggleIntroSlideAutoplay() {
    isIntroSlideAutoplay = !isIntroSlideAutoplay;
    const btn = document.getElementById("btn-slide-autoplay");

    if (isIntroSlideAutoplay) {
        if (btn) {
            btn.classList.add("playing");
            btn.innerHTML = '<i class="fa-solid fa-pause"></i> Auto-Scrolling';
        }
        startIntroSlideContinuousScroll();
    } else {
        if (btn) {
            btn.classList.remove("playing");
            btn.innerHTML = '<i class="fa-solid fa-play"></i> Auto-Scroll (Paused)';
        }
        stopIntroSlideContinuousScroll();
    }
}

function startIntroSlideContinuousScroll() {
    stopIntroSlideContinuousScroll();
    function stepScroll() {
        if (isIntroSlideAutoplay && !isSliderHoveredOrInteracting) {
            const wrapper = document.getElementById("slides-scroll-wrapper");
            if (wrapper) {
                ...
                if (wrapper.scrollLeft >= maxScroll - 2) {
                    ...
                    wrapper.scrollLeft += continuousScrollSpeed;
                }
            }
        }
        continuousScrollAnimId = requestAnimationFrame(stepScroll);
    }
    continuousScrollAnimId = requestAnimationFrame(stepScroll);
}

function stopIntroSlideContinuousScroll() {
    if (continuousScrollAnimId) {
        cancelAnimationFrame(continuousScrollAnimId);
        continuousScrollAnimId = null;
    }
}

// Initialize Continuous Smooth Scroller, Drag, & Hover Listeners
let isIntroSlideScrollerInitialized = false;

function initIntroSlideScroller() {
    if (isIntroSlideScrollerInitialized) return;
    const wrapper = document.getElementById("slides-scroll-wrapper");
    if (!wrapper) return;

    isIntroSlideScrollerInitialized = true;

    // Start slow continuous scrolling automatically
    startIntroSlideContinuousScroll();

    // Pause on hover, resume on mouse leave
    wrapper.addEventListener("mouseenter", () => {
        isSliderHoveredOrInteracting = true;
    });

    wrapper.addEventListener("mouseleave", () => {
        isSliderHoveredOrInteracting = false;
        isDown = false;
    });
    ...
    // Update active dot and counter badge on scroll
    wrapper.addEventListener("scroll", () => {
        ...
        if (counter) {
            counter.innerText = `Slide ${closestIndex + 1} / ${totalIntroSlides}`;
        }
    }, { passive: true });
    ...
    wrapper.addEventListener("mousemove", (e) => {
        ...
        wrapper.scrollLeft = initialScrollLeft - walk;
    });
}

/* ==========================================================
   MASTER 3-TAB SWITCHER LOGIC
   ========================================================== */
function switchMasterTab(index, targetAnchorId) {
    const tabBtns = document.querySelectorAll(".master-tab-btn");
    const tabPanels = document.querySelectorAll(".master-tab-panel");
    ...
    localStorage.setItem("selected_master_tab", index);
...
function initMasterTabs() {
    const savedTab = localStorage.getItem("selected_master_tab");
    if (savedTab !== null) {
        const tabIdx = parseInt(savedTab, 10);
        if (!isNaN(tabIdx) && tabIdx >= 0 && tabIdx <= 2) {
            switchMasterTab(tabIdx);
        }
    }
}

// Master init registration
document.addEventListener("DOMContentLoaded", () => {
    initMasterTabs();
    const dateInput = document.getElementById("novelLessonDate");
    if (dateInput) {
...
/* ==========================================================
   NOVEL PORTAL (THE LOST WORLD - CHAPTER 1) INTERACTIVE LOGIC
   ========================================================== */
function switchNovelTab(index) {
    const tabs = document.querySelectorAll(".novel-tab-btn");
    const panels = document.querySelectorAll(".novel-subpanel");
    tabs.forEach((t, i) => {
...
function toggleNovelVocab(el, speechMsg) {
    const ar = el.querySelector(".novel-ar-trans");
    if (ar) {
        ar.classList.toggle("show");
    }
    if (speechMsg) {
        speakText(speechMsg);
    }
...
function checkNovelInteractive(btn, isCorrect) {
    const parent = btn.parentElement;
    parent.querySelectorAll(".novel-choice-btn").forEach(b => {
    ...
    if (isCorrect) {
        ...
        speakText("Incorrect. Try again or check the model answer.");
    }
}

function toggleNovelChecklistItem(container) {
    const cb = container.querySelector('input[type="checkbox"]');
    if (cb && event.target !== cb) {
        cb.checked = !cb.checked;
    }
    container.classList.toggle("checked", cb ? cb.checked : false);
}

let isNovelStoryReading = false;
let currentNovelParaIndex = 1;

function readSingleNovelParagraph(num) {
    stopNovelSpeech();
    const p = document.getElementById("novel-p" + num);
    if (p) {
...
function playAllNovelStory() {
    stopNovelSpeech();
    isNovelStoryReading = true;
    currentNovelParaIndex = 1;
    readNextNovelInSequence();
}

function readNextNovelInSequence() {
    if (!isNovelStoryReading || currentNovelParaIndex > 10) {
        stopNovelSpeech();
        return;
    }
    document.querySelectorAll(".novel-story-para").forEach(p => p.classList.remove("reading"));
    const p = document.getElementById("novel-p" + currentNovelParaIndex);
    if (p) {
        p.classList.add("reading");
        p.scrollIntoView({ behavior: "smooth", block: "center" });
        speakText(p.innerText, () => {
            p.classList.remove("reading");
            currentNovelParaIndex++;
            if (isNovelStoryReading) {
                readNextNovelInSequence();
            }
        });
    }
...
function stopNovelSpeech() {
    if (window.speechSynthesis) {
        window.speechSynthesis.cancel();
        isNovelStoryReading = false;
        document.querySelectorAll(".novel-story-para").forEach(p => p.classList.remove("reading"));
    }
}

function resumeNovelStory() {
...
function readNovelBoard() {
    speakText("Board Organization: Chapter 1 – A Dream of Adventure. Warm-Up question: Why do some people risk everything in pursuit of dangerous adventures? Vocabulary: Adventure, Courage, Zoologist, Assignment, Deception, Remarkable, Evolution, Impulsive. Grammar: Past Simple and Past Perfect tense.");
}

function readAllNovelSummative() {
    speakText("Summative Questions: One: Explain how Gladys's idea of courage changes Malone's plans. Two: Why does Malone ask Mr. McArdle for a dangerous assignment? Three: Why does Malone write to Professor Challenger pretending to understand his work? Four: What does McArdle's reaction reveal? Five: How do Mrs. Challenger's warning and Challenger's appearance build suspense? Six: Evaluate Malone's deception. Seven: Evaluate Malone's desire for adventure.");
}

function readAllNovelJRE() {
    speakText("Justify, Reasoning and Evidence Activities. Activity 1: Is Gladys a positive influence on Malone at the beginning of the story? Activity 2: Is Malone truly ready for the kind of adventure he is seeking by the end of this chapter?");
}

function readNovelExtensiveGuide() {
    speakText("Extensive Reading Framework. Core Principle: Reading for pleasure at home, thinking for meaning, and maximizing student talking time in class. Stage 1: Reviewing and Recall. Stage 2: Viewing and Discussion with Contextual Vocabulary and Character Hot-Seating. Stage 3: Application and Pre-Viewing.");
}

function readNovelSummaryText() {
    speakText("Comprehensive Summary of The Lost World. The story follows Edward Malone's journey from romantic rejection by Gladys in London to joining Professor Challenger's daring expedition in the Amazon rainforest. After surviving treacherous climbs, prehistoric dinosaurs, and war with the ape-men, the team proves the existence of the Lost World at Queen's Hall in London.");
}

function readNovelLessonPlanText() {
    speakText("Novel Literature Lesson Plan: Reader-Response and Inquiry-Based Framework for Chapter 1: A Dream of Adventure. Stage 1: Hook and Thematic Connection. Stage 2: Context and Student-Led Recap. Stage 3: Active Exploration and Analytical Reading. Stage 4: Synthesis and Critical Reaction. Stage 5: Bridge and Foreshadowing.");
}
